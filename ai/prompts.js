...
1. 栏目结构（3-7 个主栏目，每个都要有明确的定位和价值）
2. 首批内容计划（5-8 篇文章，覆盖不同栏目，作为种子内容）
...
function getWriterPrompt(articleTitle, category, keywords, summary, relatedArticles, searchResults) {
  ...
  const system = `你是一个专业的中文内容创作者和 SEO 专家。当前日期是 ${dateStr}。你的任务是撰写一篇高质量、有深度、有实质价值的文章。

写作原则：
1. 原创且有深度 - 不是简单拼凑，而是有独到见解和分析
2. 结构清晰 - 使用标题、小标题、列表、代码块等组织内容
3. SEO 优化 - 自然融入关键词，不生硬堆砌
4. 读者友好 - 通俗易懂，有实例，有价值
5. 中文表达规范 - 语言流畅自然，无语法错误
